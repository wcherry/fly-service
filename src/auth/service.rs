...
use crate::shared::dto::{NewUserDto, UserDto, CreateUser};
...
pub fn get_user(conn: &mut SqliteConnection, user_id: i32) -> Result<UserDto, DbError> {
    let user = users::dsl::users
        ...
        .select((users::id, users::username, users::email_address, users::active))
        .first::<UserDto>(conn)?;    
    Ok(user)
}
...
pub fn find_user_by_username_and_password(conn: &mut Connection, username: String, password: String) -> Result<UserDto, DbError> {
    let user = users::dsl::users
        ...
        .select((users::id, users::username, users::password, users::email_address, users::active))
        .first::<CreateUser>(conn)?;    
...
pub fn create_user(
    ...
    // create default lists (personal, work, etc.) for the new user
    // Add a initial task to the personal list to learn how to use task_now
